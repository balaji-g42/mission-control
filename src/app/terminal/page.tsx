...
import { ChevronLeft, Terminal as TerminalIcon, Wifi, WifiOff, AlertCircle } from 'lucide-react';
...
export default function TerminalPage() {
  ...
  const connectToServer = async () => {
    ...
    addLog('Initializing terminal connection...', 'info');

    // Use the configured WebSocket URL (gateway)
    const wsUrl = TERMINAL_WS_URL;

    addLog(`Connecting to OpenClaw Gateway at ${wsUrl}...`, 'info');
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        addLog('Waiting for gateway challenge...', 'info');
      };
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === 'event' && data.event === 'connect.challenge') {
            addLog('Challenge received, authenticating...', 'info');
          ...
          if (data.type === 'res' && data.payload && data.payload.session_id && !sessionId) {
            ...
            addLog('Terminal session created: ' + sessionId, 'success');
            addLog('Loading terminal interface...', 'info');
      ...
      ws.onerror = () => {
        ...
        addLog('Connection failed - gateway may be offline', 'error');
        setIsConnecting(false);
      };
    } catch (err) {
                ...
                <div className="text-center max-w-md">
                  {!TERMINAL_ENABLED ? (
                    ...
                    <>
                      ...
                      <p className="text-[#8b949e] mb-6">Connect to the terminal server to access the command line</p>
                      <button
